...
#ifndef NETWORKEDU_IP_H
#define NETWORKEDU_IP_H

/* 関数プロトタイプ宣言 */
void print_ip(struct ip *ip);
int IpRecvBufInit();
int IpRecvBufAdd(u_int8_t id);
int IpRecvBufDel(u_int8_t id);
int IpRecvBufSearch(u_int8_t id);
...
int IpSend(int soc, struct in_addr *saddr, struct in_addr *daddr, u_int8_t proto, int dontFlagment, int ttl, int len);


#endif //NETWORKEDU_IP_H

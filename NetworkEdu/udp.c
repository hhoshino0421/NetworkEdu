...
int UdpSocket(u_int16_t port) {
    ...
    if (port == DHCP_CLIENT_PORT) {
        printf("UdpSocket:port %d can not use\n", port);
        return -1;
    }
...
int UdpSendLink(int soc, u_int8_t smac[6], u_int8_t dmac[6], struct in_addr *saddr, struct in_addr *daddr
    ...
    IpSendLink(soc, smac, dmac, saddr, daddr, IPPROTO_UDP, dontFlagment, Param.IpTTL, ptr - sbuf);
    print_udp(udp);
    print_hex(data, len);
    printf("]\n");
...
int UdopSend(int soc, struct in_addr *saddr, struct in_addr *daddr,u_int16_t sport,u_int16_t dport

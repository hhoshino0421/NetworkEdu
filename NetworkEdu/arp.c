...
void print_ether_arp(struct ether_arp *ether_arp) {
    ...
    printf("---ether_arp---\n");
    printf("arp_hdr=%u", ntohs(ether_arp->arp_hrd));
    ...
    printf("arp_hls=%u,", ether_arp->arp_hln);
    ...
    printf("arp_spa=%s\n", my_ether_ntoa_r(ether_arp->arp_spa, buf1));
...
int ArpAddTable(u_int8_t mac[6], struct in_addr *ipaddr) {
    ...
    for (i = 0;i < ARP_TABLE_NO; i++) {
        ...
        if (memcmp(ArpTable[i].mac, AllZeroMac, 6) == 0) {
            ...
            if (ArpTable[i].timestamp < oldsetTime) {

                oldsetTime = ArpTable[i].timestamp;
                oldestNo = i;
            }
        }
    ...
    for (i = 0; i < ARP_TABLE_NO; i++) {
        ...
        if (memcmp(ArpTable[i].mac, AllZeroMac, 6) == 0) {
            ...
            if (ArpTable[i].ipaddr.s_addr == ipaddr->s_addr) {
                ...
                memcpy(ArpTable->mac, AllZeroMac, 6);
